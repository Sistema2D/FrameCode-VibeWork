...
5. Checkpoint discovery with visited scope, unmapped surfaces, blocked roles, pending states, remaining time/attempt budget and next frontier action. Use the frontier table. Mark inventory complete only when the declared bounded scope is mapped and frontier resolved. If budget expires or a tool repeatedly fails, save in-progress state and resume it on the next execution. An initial checkpoint with no mapped surfaces is valid when all access is blocked; it cannot be a completed inventory or a test pass. Mapping completeness is separate from functional success.
...
10. After initial mapping, scope subsequent runs to affected surfaces and dependencies. New controls, commands, endpoints, hardware revisions or operating modes reopen inventory gaps. Source changes, defect reports or due review dates trigger selective maintenance. If the behavior contract changed, review and rerun affected cases; do not transfer an old pass to a new hash. Store screenshots/traces outside default context. A checker reference documents a claimed question; only a host-linked conversation event can corroborate that the user was actually asked or answered.
