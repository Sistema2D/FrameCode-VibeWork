...
The governed source and every downloaded template keep the conventional single-tree root described above. FCVW has no multilingual runtime layout and does not create, discover, select, or synchronize language directories during normal validation, instantiation, or use.
...
`tools/package_release_fcvw.py` validates the external staging tree, creates deterministic language-specific ZIPs with one conventional template root, inspects their manifests, and writes `SHA256SUMS.txt`. Its explicit `--allow-in-review` mode creates local candidate assets without weakening or satisfying the publication review gate.
