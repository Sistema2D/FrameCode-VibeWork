...
class DocumentGraphTests(TemporaryRootTest):
    ...
    def test_task_list_marker_requires_valid_state(self) -> None:
        _, root = self.make_root()
        self.write_entrypoints(root)
        (root / "FCVW" / "note.md").write_text("- [] Broken task\n", encoding="utf-8")
        findings = build_graph(root).findings
        self.assertTrue(
            any(
                item.rule == "document-task-list-syntax" and item.path == "FCVW/note.md"
                for item in findings
            )
        )
